...
if st.button("Predict"):
    ...
    # 1. Create a DataFrame for numerical features with EXACT names
    num_cols = [
        "NUMACTIVELastYrQTRS", "TOTALSESSIONLENGTHLastYr", "TOTALNUMSESSIONSLastYr",
        "NUMACTIVITYDAYSLastYr", "SUBSCRIPTIONTIMEINLastYr", "SUBSCRIPTIONTIMEINThisYr",
        "SESSIONLENGTHPERDAYLastYr", "AVG_SESSIONSPERACTIVEQTR", "AGE", "TECH_COMFORT_SCORE"
    ]
    num_df = pd.DataFrame([
        [num_active_qtrs, total_session_length, total_num_sessions,
         num_activity_days, subscription_time_LastYr, subscription_time_ThisYr,
         session_length_per_day, avg_sessions_per_qtr, age, tech_comfort_score]
    ],
    columns=num_cols)

    # 2. Encode categorical features
    cat_cols_in = ["EDUCATION", "INCOME_LEVEL", "DEVICE_TYPE"]
    cat_data = pd.DataFrame([[education, income_level, device_type]], columns=cat_cols_in)
    encoded_cat = encoder.transform(cat_data)
    encoded_df = pd.DataFrame(encoded_cat, columns=encoder.get_feature_names_out())

    # 3. Concatenate and pass to model
    input_df = pd.concat([num_df, encoded_df], axis=1)
